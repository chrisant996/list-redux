...
newaction {
    trigger = "release",
    description = "List: Creates a release of List",
    execute = function ()
        ...
        local build_code = function (target)
            if have_msbuild then
                ...
                x64_ok = exec(have_msbuild .. " /m /v:q /p:configuration=release /p:platform=x64 list.sln /t:" .. target)
        ...
        -- Copy release files.
        copy(src .. "/x64/list.exe", root_dir)
        copy(src .. "/x64/list.pdb", root_dir)

        -- Package the release and the pdbs separately.
        os.chdir(src .. "/x64")
        if have_7z then
            exec(have_7z .. " a -r  " .. target_dir .. "list-redux-v" .. version .. "-symbols.zip  *.pdb")
            exec(have_7z .. " a -r  " .. target_dir .. "list-redux-v" .. version .. ".zip  *.exe")
        end
...
newaction {
    trigger = "manifest",
    description = "List: generate app manifest",
    execute = function ()
        toolchain = _OPTIONS["vsver"] or "vs2022"
